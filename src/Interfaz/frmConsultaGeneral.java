...
public class frmConsultaGeneral extends javax.swing.JInternalFrame {
    ...
    void llenarDatos() {
        if(sql.versiesta("call buscar_preclinica("+cuentap+")")){
        JTextField A[] = {
        ...
        JOptionPane.showMessageDialog(null, "El paciente debe pasar antes a Preclinica");
    ...
    void llenarTablaR() {
        sql.RegistrosTabla("call resumenes_CG(3," + iddoc + "," + idpaciente + ",'','','','',0,0,0,0,0,0,0)", modelo_R);
    }
    ...
    private void initComponents() {
        ...
        tablaR.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "FECHA", "DOCTOR", "IMPRESION DIAGNOSTICA", "INDICACIONES", "HEA"
            }
        ));
        ...
        if (tablaR.getColumnModel().getColumnCount() > 0) {
            tablaR.getColumnModel().getColumn(0).setMinWidth(40);
            tablaR.getColumnModel().getColumn(0).setMaxWidth(30);
            tablaR.getColumnModel().getColumn(1).setMinWidth(100);
            tablaR.getColumnModel().getColumn(1).setMaxWidth(100);
            tablaR.getColumnModel().getColumn(2).setMinWidth(200);
            tablaR.getColumnModel().getColumn(2).setMaxWidth(200);
        }
        ...
        NOTA.setBackground(new java.awt.Color(255, 223, 0));
        NOTA.setFont(new java.awt.Font("Trebuchet MS", 1, 14)); // NOI18N
        NOTA.setText("NOTA");
        NOTA.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 0, 0), 4));
        ...
        jLabel84.setText("HEA");

        txtobs.setColumns(20);
        txtobs.setFont(new java.awt.Font("Trebuchet MS", 0, 18)); // NOI18N
        txtobs.setRows(5);
        txtobs.setBorder(null);
        jScrollPane15.setViewportView(txtobs);
            ...
            .addGroup(JPResumenLayout.createSequentialGroup()
                ...
                .addGroup(JPResumenLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(JPResumenLayout.createSequentialGroup()
                        .addGroup(JPResumenLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(JPResumenLayout.createSequentialGroup()
                                .addComponent(NOTA, javax.swing.GroupLayout.PREFERRED_SIZE, 132, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(279, 279, 279)
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(JPP, javax.swing.GroupLayout.DEFAULT_SIZE, 743, Short.MAX_VALUE)
                .addContainerGap())
        );
    ...
    private void btnguardarRActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnguardarRActionPerformed
        String pe=this.txtpeso.getText();
        String ta=this.txttalla.getText();
        String temp=this.txttemperatura.getText();
        String imc=this.txtimc.getText();
        String pul=this.txtpulso.getText();
        String pa=this.txtpa.getText();
        String edad=this.txtedad.getText();
        String indicaciones = txtIndica.getText(), diagno = txtImpreDiag.getText(),obs=txtobs.getText();
        if (!indicaciones.isEmpty() && !diagno.isEmpty() && !obs.isEmpty()) {
            sql.EjecutarConsultas("call resumenes_CG(1," + iddoc + "," + idpaciente + ",'" + diagno + "','"
                    + va.getfecha() + "','" + indicaciones + "','" + obs + "',"+pe+","+ta+","+temp+","+imc+","+pa+","+pul+","+edad+")");
            llenarTablaR();
            limpirResumenes();
            bloquearResu(false);
        } else {
    ...
    private void tablaRMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablaRMouseClicked
        if (modelo_R.getValueAt(tablaR.getSelectedRow(), 3) != null) {
            id_nota=modelo_R.getValueAt(tablaR.getSelectedRow(), 0).toString();
            txtImpreDiag.setText(modelo_R.getValueAt(tablaR.getSelectedRow(), 3).toString());
            txtIndica.setText(modelo_R.getValueAt(tablaR.getSelectedRow(), 4).toString());
            txtobs.setText(modelo_R.getValueAt(tablaR.getSelectedRow(), 5).toString());
            medico=modelo_R.getValueAt(tablaR.getSelectedRow(), 2).toString();
    ...
    private void NOTAActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_NOTAActionPerformed
        // TODO add your handling code here:
        String nombre= this.txtnombre.getText()+" "+this.txtapellido.getText();
        String sexo=this.cmbsexo.getSelectedItem().toString();
        try{
            sql.conec();
            String url = "C:/temp/nota_evolucion.jasper";            
            JasperReport reporte= (JasperReport)JRLoader.loadObject(url);
            Map parametro=new HashMap();   
            parametro.put("idr", this.id_nota);
            parametro.put("Doctor", medico);
            parametro.put("paciente", nombre);
            parametro.put("cuenta", cuentap);
